...
L_values = zeros(length(k_values), length(mu_a_values));
...
for c = 1:length(mu_a_values)
    plot(k_values, L_values(:, c), 'DisplayName', sprintf('mu_a=%d, mu_e=%d', mu_a_values(c), mu_e_values(c)));
end
hold off;
...
ylabel('L');
legend('show');
title('L vs k for different values of mu_a and mu_e with the same index');
